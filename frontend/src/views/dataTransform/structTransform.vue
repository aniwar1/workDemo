  ...
  <div class="page-container">
      ...
      <el-table :data="tableData" v-loading="loading" stripe>
        ...
        <el-table-column label="操作" width="240">
          <template #default="{ row }">
            ...
            <el-button link type="info" @click="handleViewResult(row)">预览</el-button>
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="600px">
      <el-form ref="formRef" :model="form" :rules="rules" label-width="100">
        ...
        <el-form-item label="输入文件" v-if="!form.id">
          <el-upload ref="uploadRef" :auto-upload="false" :limit="1" accept=".csv,.json,.xml" :on-change="onFileChange">
            <el-button type="primary">选择文件</el-button>
            <template #tip><div class="upload-tip">支持 CSV, JSON, XML 格式</div></template>
          </el-upload>
        </el-form-item>
        <el-form-item label="输出路径">
          <el-input v-model="form.outputPath" placeholder="输出文件名，如 output.json" />
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    <el-dialog v-model="resultDialogVisible" title="转换结果" width="600px">
      <el-alert v-if="executeResult" type="success" :closable="false" style="margin-bottom: 12px">
        转换完成：{{ executeResult.nodes }} 个节点，{{ executeResult.relations }} 个关系
      </el-alert>
      <el-alert v-if="executeResult?.error" type="danger" :title="executeResult.error" :closable="false" />
      <el-empty v-if="!executeResult" description="暂无结果" />
    </el-dialog>
  </div>
</template>
...
const form = reactive({ id: null, name: '', sourceType: '', targetFormat: 'kg_json', inputPath: '', outputPath: '' })
...
const handleAdd = () => {
  Object.assign(form, { id: null, name: '', sourceType: '', targetFormat: 'kg_json', inputPath: '', outputPath: '' })
...
const handleExecute = async (row) => {
  ...
  try {
    const res = await executeTransform(row.id)
    executeResult.value = res.data
    if (!executeResult.value?.error) {
      ElMessage.success('转换完成')
    } else {
      ElMessage.error('转换失败: ' + executeResult.value.error)
    }
    loadData()
  } catch (e) {
...
const handleViewResult = async (row) => {
  executeResult.value = { nodes: row.recordCount || 0, relations: row.recordCount || 0 }
  resultDialogVisible.value = true
}
