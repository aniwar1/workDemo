...
│  kg_model            — 图谱模型
│  kg_corpus           — 语料
│  kg_node_instance    — 节点实例（含 sourceTaskId / neo4jId 关联字段）
│  kg_edge_instance    — 边实例（含 sourceTaskId 关联字段）
│
└─► Neo4j（实体关系存储 / 图谱层）
   │  Entity 节点 — 存储图谱实体（name / entityType / graphId / properties）
   │  RELATION 边 — 存储实体间关系（relationType / graphId）
   │
   └─► MinIO（文件存储）
      │  uploads/  — 通用上传文件
      │  corpus/   — 语料文件
...
双写流程：
  LLM 抽取结果
       │
       ▼
  Neo4jService.batchCreateNodesAndRelations()
       │  MERGE Entity 节点（返回 Neo4j 原生 ID）
       │  MERGE RELATION 边
       │
       ▼
  KgNodeInstanceService.batchSaveWithNeo4jIds()
       │  写入 kg_node_instance（带 neo4jId + sourceTaskId）
       │
       ▼
  KgEdgeInstanceService.batchSave()
       │  写入 kg_edge_instance（带 sourceTaskId）
       │
       ▼
  MySQL + Neo4j 双存储完成
...
│  维度       │  MySQL                                       │  Neo4j                                      │
├─────────────┼──────────────────────────────────────────────┼──────────────────────────────────────────────┤
│  定位       │  业务元数据 + 任务管理                        │  知识图谱本体存储与查询                      │
│  节点存储   │  kg_node_instance（关联信息、来源追踪）       │  Entity 节点（name, entityType, graphId）   │
│  边存储     │  kg_edge_instance（关联信息、来源追踪）       │  RELATION 关系（relationType, graphId）      │
│  路径查询   │  —                                           │  最短路径、路径发现                         │
│  图分析     │  —                                           │  度统计、关系类型枚举                       │
│  存储优势   │  事务性、关联查询、业务报表                   │  图遍历性能、多跳查询、路径分析             │
└─────────────┴──────────────────────────────────────────────┴──────────────────────────────────────────────┘
```
...
#### 知识抽取 `/api/data/extract`

| 方法 | 路径 | 说明 | 认证 |
|------|------|------|------|
| POST | `/direct` | 直接抽取（同步，无需创建任务） | 是 |
| POST | `/direct-save` | 直接抽取并保存到图谱（同步，双写 Neo4j + MySQL） | 是 |
| GET | `/task/list` | 分页查询抽取任务 | 是 |
| POST | `/task` | 创建抽取任务 | 是 |
| POST | `/task/{id}/start` | 启动抽取任务 | 是 |
| POST | `/task/{id}/stop` | 停止抽取任务 | 是 |
| GET | `/task/{id}/result` | 获取抽取任务结果 | 是 |
| DELETE | `/task/{id}` | 删除抽取任务 | 是 |
...
| extractType | string | all | 抽取类型：entity=仅实体，relation=仅关系，all=全部 |
| language | string | zh | 文本语言：zh=中文，en=英文 |
| model | string | — | 使用的 LLM 模型名称，留空则使用配置默认模型（qwen-plus） |
| schema | string | — | 自定义 schema 约束，如 "人物,公司,产品,地点" |

**`POST /direct-save` 请求体：**
```json
{
  "text": "待抽取的文本内容",
  "extractType": "all",
...
| source_task_id | BIGINT | 来源抽取任务 ID（双存储关联） |
| neo4j_id | BIGINT | Neo4j 原生节点 ID（双存储关联） |
...
| id | BIGINT PK | 边 ID |
| graph_id | BIGINT | 图谱 ID |
| source_node_id | BIGINT | 源节点 ID（对应 MySQL kg_node_instance.id） |
| target_node_id | BIGINT | 目标节点 ID（对应 MySQL kg_node_instance.id） |
...
|用户输入文本，点击「抽取」
    │
    ▼
POST /api/extract/direct  或  POST /api/extract/direct-save
    │
    ▼
DashScopeService.extractDirect() → LLM 抽取实体 + 关系
    │
    ▼
├─ /direct: 返回 LlmExtractResult { entities, relations }
│
└─ /direct-save:
    │
    ▼
  ...
  KgEdgeInstanceService.batchSave()
    ...
    │
    ▼
  返回 { llmResult, nodeCount, edgeCount }
```
...
| 调整项 | 说明 |
|--------|------|
| 路由层级重组 | 按「平台管理 / 图谱项目管理 / 知识抽取与转化 / 图谱实例管理」四大模块重组 |
| KOS 路径扁平化 | 基于KOS知识抽取 → 深度学习知识抽取（移除一层 `/llm/extract` 嵌套） |
| 标注管理路径 | `/data/kos/dl/preprocess/annotation-manage` → `/data/kos/dl/preprocess/manage` |
| LLM 路径扁平化 | `/data/llm/extract/...` → `/data/llm`（直接渲染页面） |
| 新增页面 | `extractIndex.vue`（知识抽取）、`multimodalUpdate.vue`（多模态数据更新） |

#### 后端接口变更

| 调整项 | 说明 |
|--------|------|
| 抽取接口路径变更 | `/api/extract` → `/api/data/extract` |
| 新增接口 | `POST /api/data/multimodal/*`（多模态数据更新，待实现） |
| 新增接口 | `POST /api/annotation/auth/*`（标注授权，待实现） |

#### 新增功能

| 功能 | 说明 |
|------|------|
| MySQL + Neo4j 双存储架构 | 抽取结果同时写入 Neo4j（图谱本体）和 MySQL（图谱节点/边实例表），实现业务追踪与图谱查询双重能力 |
| 直接抽取并保存图谱接口 | `POST /api/extract/direct-save`：抽取文本并指定目标图谱 ID，一键完成 LLM 抽取 + Neo4j 写入 + MySQL 双写 |
| 知识抽取页面同步化 | 移除异步任务机制，改为同步直接抽取，前端页面简化为输入文本 + 抽取结果展示，API 精简为 2 个接口 |

#### 重大调整

| 调整项 | 说明 |
|--------|------|
| 知识抽取页面重构 | 移除历史记录 Tab、图谱预览 Tab、自定义提示词、语料选择器等功能，改为简洁的同步抽取流程 |
| 知识抽取 API 精简 | 删除 9 个任务管理相关接口（list/dl/llm/start/stop/result/graph-data/delete/getById），保留 2 个直接抽取接口（direct/direct-save） |
...
> **MyBatis-Plus 保留字注意事项**：当 Java 字段名与 MySQL 保留字相同时，MyBatis-Plus 会在 SELECT 中生成 `AS xxx`（使用 Java 字段名作为别名），导致 SQL 语法错误。解决方式：将 Java 字段名改为非保留字，用 `@TableField` 指定数据库列名。
